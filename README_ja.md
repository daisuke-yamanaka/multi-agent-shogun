...
```markdown
## 進行中
| ワーカー | タスク | フェーズ | 状態 | レビュー |
|----------|--------|---------|------|---------|
| 足軽 1 | React調査 | design | 実行中 | - |
| 足軽 2 | Vue調査 | design | 実行中 | - |
| 足軽 3 | Angular調査 | design | 完了 | 足軽4 レビュー中 |
```
...
全ての重要な成果物に対して、3フェーズのレビュープロトコルを実施：

| フェーズ | 対象 | レビュアー |
|---------|------|-----------|
| Phase 1: 計画 | 家老のタスク分解案 | 足軽1名 |
| Phase 2: 作業 | 設計/実装/テスト成果物 | 作業者以外の足軽 |
| Phase 3: 統合 | 家老の統合計画 | 足軽1名 |
...
自己レビュー禁止 — レビュアーは必ず作業者と異なる足軽が担当。品質ゲートとして機能する。
...
1. **品質ゲート**: 設計・実装・テストの各フェーズでレビューを挟み、問題を早期発見
2. **自己レビュー禁止**: 作業者と異なる足軽がレビューすることで客観性を担保
3. **家老の計画もレビュー対象**: 家老のタスク分解案と統合計画も足軽がレビューし、盲点を排除
4. **並列性の維持**: レビュー待ちでも他の足軽は別タスクを進行可能
5. **サイクル上限**: 同一成果物のrequest_changesは最大2回。3回目はエスカレーションで無限ループを防止
...
実行される処理:
1. 将軍が家老に委譲
2. 家老がdashboard.mdに実行計画を記載 → 足軽1名が計画をレビュー
3. レビュー承認後、家老が割り当て:
...
4. 5体が同時に調査
5. 各成果物を別の足軽がピアレビュー → 承認
6. 結果がdashboard.mdに集約
```
...
│   ├── tasks/                # 各ワーカーのタスクファイル（phase, review_ofフィールド含む）
│   └── reports/              # ワーカーレポート（review_resultフィールド含む）
│
